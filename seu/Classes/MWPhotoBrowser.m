...
- (MWCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    ...
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapCaption:)];
    [captionView addGestureRecognizer:tap];
    captionView.userInteractionEnabled = YES;
    captionView.tag = index;
    return captionView;
}

- (void)tapCaption:(UITapGestureRecognizer *)tap {
    NSLog(@"called here");
    if ([tap.view isKindOfClass:[MWCaptionView class]] && [_delegate respondsToSelector:@selector(photoBrowser:didTapCaptionViewAtIndex:)]) {
        [_delegate photoBrowser:self didTapCaptionViewAtIndex:tap.view.tag];
    }
}
